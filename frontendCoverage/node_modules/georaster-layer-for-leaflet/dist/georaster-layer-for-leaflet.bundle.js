...
var _pixelUtils = require("pixel-utils");
var _densePolygon = _interopRequireDefault(require("bbox-fns/dense-polygon.js"));
var _isUTM = _interopRequireDefault(require("utm-utils/src/isUTM.js"));
var _getProjString = _interopRequireDefault(require("utm-utils/src/getProjString.js"));
var _proj4Collect = _interopRequireDefault(require("proj4-collect"));
var _reprojectGeojson = _interopRequireDefault(require("reproject-geojson"));
var _merge = _interopRequireDefault(require("bbox-fns/merge.js"));
var _polygon = _interopRequireDefault(require("bbox-fns/polygon.js"));
var _fastMin = _interopRequireDefault(require("fast-min"));
var _fastMax = _interopRequireDefault(require("fast-max"));
var _geoExtent = require("geo-extent");
var _geowarp = _interopRequireDefault(require("geowarp"));
var _geowarpCanvas = _interopRequireDefault(require("geowarp-canvas"));
var _snapBbox = _interopRequireDefault(require("snap-bbox"));
var _web = require("georaster-stack/web");
...
const EPSG4326 = 4326;
const ORIGIN = [0, 0];
const geowarp = (0, _geowarpCanvas.default)(_geowarp.default);
...
const measureSkew = (bbox, reproj) => {
  const poly = (0, _polygon.default)(bbox);
...
const GeoRasterLayer = L.GridLayer.extend({
  ...
  initialize: function (options) {
    this.proj4 = (0, _proj4Collect.default)();
    try {
      ...
      this.subextents = this.georasters.map(g => new _geoExtent.GeoExtent([g.xmin, g.ymin, g.xmax, g.ymax], {
        srs: g.projection
      }));
      ...
      this.simpleExtent = new _geoExtent.GeoExtent([0, 0, max_width, max_height]);
      ...
      this.stack = _web.GeoRasterStack.init({
  ...
  getExtent: function (srs = 4326) {
    ...
    if (!this._extent[srs]) {
      this._extent[srs] = new _geoExtent.GeoExtent((0, _merge.default)(this.subextents.map(extent => extent.reproj(srs).bbox)), {
        srs
      });
    }
    ...
    if (options.mask && options.mask !== "auto") {
      ...
      if (projections.size === 1) {
        this.mask = Promise.resolve({
          ...
          geometry: {
            type: "MultiPolygon",
            coordinates: this.subextents.map(ext => ext.unwrap()).flat().map(ext => (0, _densePolygon.default)(ext.bbox, {
              density: 100
            }))
          }
        });
        ...
        this.mask = Promise.resolve({
          ...
          geometry: {
            ...
            coordinates: this.subextents.map(ext => ext.unwrap()).flat().map(ext => ext.reproj(4326, {
              density: "high"
            })).map(ext => (0, _densePolygon.default)(ext.bbox, {
              density: 100
            }))
          }
        });
  ...
  getProjectionString: function (proj) {
    if ((0, _isUTM.default)(proj)) return (0, _getProjString.default)(proj);
  ...
  drawTile: function ({
    ...
    try {
      ...
      let extentOfLayer;
      if (inSimpleCRS) {
        extentOfLayer = new _geoExtent.GeoExtent(this.getBounds(), {
          srs: "simple"
        });
      ...
      if (new Set(this.georasters.map(g => JSON.stringify([g.pixelHeight, g.pixelWidth, g.projection, g.xmin, g.ymin, g.xmax, g.ymax]))).size === 1 && (skew[0] === 0 && skew[1] === 0 || inDefaultCRS && [4326, 3857].includes(this.georasters[0].projection))) {
        ...
        const snapped = (0, _snapBbox.default)({
        ...
        const newExtentOfInnerTileInRasterCRS = new _geoExtent.GeoExtent(snapped.bbox_in_coordinate_system, {
      ...
      setTimeout(async () => {
        try {
          ...
          if (this.calcStats) {
            ...
            if (this._dynamic) {
              const rawToRgbFn = _pixelUtils.rawToRgb.default || _pixelUtils.rawToRgb;
              try {
          ...
          if (this.debugLevel >= 4 && inSimpleCRS === false && mask) {
            try {
              const geojson = await (0, _reprojectGeojson.default)(mask, {
                from: this.mask_srs,
                to: 4326
              });
              L.geoJSON(geojson, {
          ...
          const theoretical_min = this.calcStats ? (0, _fastMin.default)(this.currentStats.mins) : undefined;
          const theoretical_max = this.calcStats ? (0, _fastMax.default)(this.currentStats.maxs) : undefined;
          const in_stats = (() => {
            if (this.stats) {
  ...
  _tileCoordsToExtent: function (coords, debug = false) {
    ...
    return new _geoExtent.GeoExtent(bbox, {
      srs: crs.code
    });
  },
  getTileExtent: function (coords, debug = false) {
    ...
    if (isSimpleCRS(crs)) {
      ...
      return new _geoExtent.GeoExtent(bounds, {
        srs: "simple"
      });
  ...
  isSupportedProjection: function (proj) {
    return (0, _isUTM.default)(proj) || proj in this.proj4.defs || `EPSG:${proj}` in this.proj4.defs;
  },
  initBounds: function (options) {
    ...
    if (!this._bounds) {
      ...
      if (isSimpleCRS(map_crs)) {
        ...
        const [xmin, ymin, xmax, ymax] = (0, _merge.default)(bboxes_in_map_crs);
...
var _default = exports.default = GeoRasterLayer; // Explicitly exports public types